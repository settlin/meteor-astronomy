...
export async function classUpdateAsync(args = {}) {
  const {
    className,
    selector,
    modifier,
  ...
  let oldDocs;
  if (options.multi) {
    docs = await Class.find(selector, options).fetchAsync();
    ...
    oldDocs = _mapKeys(await Class.find(selector, _extend(options, {
      defaults: false
    })).fetchAsync(), oldDoc => oldDoc._id);
  }
  else {
    docs = await Class.find(selector, _extend(options, {
      limit: 1
    })).fetchAsync();
    // Get all old documents in one batched query so that performance is
    // improved when updating many documents. Map all documents to an object
