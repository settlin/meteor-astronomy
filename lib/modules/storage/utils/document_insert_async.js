...
export async function documentInsertAsync(args = {}) {
  const {
    doc,
    stopOnFirstError,
    fields,
  ...
  await triggerBeforeSave(args);
  await triggerBeforeInsert(args);
  ...
  try {
    ...
    await triggerAfterInsert(args);
    await triggerAfterSave(args);
