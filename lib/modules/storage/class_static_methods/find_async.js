...
function createMethod(methodName) {
  return async function(selector, options) {
    ...
    if (!options.disableEvents) {
      await Class.dispatchEvent(new Event('beforeFind', {
        selector,
        options
      }));
    }
    ...
    if (options.transform !== null && classTransform !== null) {
      ...
      options.transform = wrapTransform({
		...
		const res = Collection[methodName](selector, options);
		let result = await (methodName === 'find' ? res.fetchAsync() : res);

    // Modify a query result using the "afterFind" event handlers.
    if (!options.disableEvents) {
      await Class.dispatchEvent(new Event('afterFind', {
        selector,
        options,
        result
      }));
...
const findFetchAsync = createMethod('find');
const findOneAsync = createMethod('findOneAsync');

export { findFetchAsync, findOneAsync };